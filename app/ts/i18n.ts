...
const zhCN: Record<string, string> = {
  ...
  'insp.master': '母版 (master)', 'insp.noMaster': '（无母版）', 'insp.transition': '切换 (transition)',
  'insp.bg': '背景色', 'insp.notes': 'notes 演讲备注',
  'insp.anims': '动画编排（按顺序播放）', 'insp.noAnims': '暂无动画。点击添加，放映时按 trigger 编排。',
  'insp.animAdd': '＋添加动画', 'insp.animDel': '删除此动画', 'insp.target': '目标 (target)', 'insp.effect': '效果 (effect)', 'insp.trigger': '触发 (trigger)', 'insp.duration': '时长 (ms)',
  ...
  'insp.custom': '自定义路径 (custom path)',
  'insp.sel': '（无）', 'insp.tableDefault': '（默认）', 'insp.autoWrap': '自动换行', 'insp.noWrap': '不换行',
  // 通用属性名
  'p.fill': '填充 (fill)', 'p.stroke': '描边 (stroke)', 'p.shadow': '阴影 (shadow)',
  // toast
  ...
  't.imgSrc': '图片路径（相对 deck 目录，或 https://…）',
};
