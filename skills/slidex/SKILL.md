...
Use this skill with an installed `slidex` CLI or a built SlideX checkout (`node /path/to/slidex/dist/cli.js`). The skill supplies instructions, not a bundled runtime. To install the public CLI after its first release, use `npm install -g @xiahan/slidex@next` (Node.js 22); the versioned CLI `.tgz` attached to a GitHub Release is another option. Do not install software without authorization. Read [authoring.md](references/authoring.md) when writing DSL or choosing an export mode.

## Workflow

1. Locate the CLI and run `slidex version` / `slidex help`. Do not assume the working directory is the SlideX repository or that the unscoped npm package `slidex` is this project. If unavailable, report the missing runtime; do not substitute an unrelated presentation format.
...
- `slidex inspect <file>` shows the project version and IDs. For an authorized incremental edit, read `docs/ai-patch.md` in the installed CLI package or source checkout, use `slidex patch <file> <patch.json> --dry-run`, then apply only if the version and proposed changes match the intended edit.
