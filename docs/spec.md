...
- 渲染使用 KaTeX：正式编辑器/Viewer 本地打包，服务端渲染、打印及独立 HTML 内联随应用分发的运行时和字体。用户自建渲染环境未加载运行时时仍可回退；`W_KATEX_OFFLINE` 保留为显式环境声明的兼容诊断，见 §17。
...
- `fill` 默认 `#1A1A1A`。
- 通过随应用分发的 Font Awesome CSS/字体渲染字形，`w`/`h` 决定字号（取小值）。正式编辑器与导出不依赖其 CDN；用户自建渲染环境仍需自行提供字体资源。
...
| `html` | 单文件放映包 | 内联 CSS/JS/本地媒体、KaTeX、Font Awesome 及其字体；项目内字体样式与字体文件可内联，用户显式配置的远程资源仍需网络 |

导出前等待条件：字体加载（`document.fonts.ready`）+ KaTeX 渲染完成标记 + 图片 `complete`，最多 10s。

---

*当前能力边界：单文件 XML 或 include 多文件页面项目（路径、ID 与安全保存规则见 [大型项目](large-projects.md)）；36 种内置形状及 custom path；8 类 SVG 图表；原生图表、动画与切换支持明确子集。PPTX 同时提供整页图与原生/混合可编辑导出，差异由能力报告列出。*
