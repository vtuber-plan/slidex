...
格式设计参考了 Kimi PPTD（YAML）的成熟模型——调色板/文本样式/表格样式/元素几何——载体换为 XML，并做了取舍（见 §11 与 roadmap）。
...
- **单一主文件**：所有页面内联在 `.slx` 中，不拆分页文件（与 PPTD 的多文件方案不同——XML 单文件可读性更好，且方便 LLM 整体生成）。
...
对富文本最终样式，按下表**自上而下**取第一个有值者（与 PPTD 相同的模型）：
...
**形状不支持内嵌文字**（与 PPTD 一致）：加一个叠放的 `<text>`。
...
合并规则与 PPTD 相同：被合并覆盖的格**直接省略**，不写占位。单元格边框默认取表样式 `cell` 的设定。
...
*附：与 PPTD 的差异速览 —— YAML→XML；多文件→单文件；177 形状→10 内置+custom（可扩）；13 图表→5 类纯 SVG（可扩）；动画/切换→v1 不含（见 roadmap）；PPTX 原生映射→v1 图像页（roadmap 提供原生文本/形状映射）。*
